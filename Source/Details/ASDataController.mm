...
- (void)updateWithChangeSet:(_ASHierarchyChangeSet *)changeSet
{
  ...
  id layoutContext;
  {
    as_activity_scope(as_activity_create("collectionUpdateLatchNewData", changeSet.rootActivity, OS_ACTIVITY_FLAG_DEFAULT));
    as_log_debug(ASCollectionLog(), "ChangeSet: %@", changeSet);
  ...
  dispatch_group_async(_editingTransactionGroup, _editingTransactionQueue, ^{
    as_activity_scope(as_activity_create("collectionUpdatePrepare", changeSet.rootActivity, OS_ACTIVITY_FLAG_DEFAULT));
    ...
    [self batchAllocateNodesFromElements:elementsToProcess andLayout:(! canDelegateLayout) batchSize:elementsToProcess.count batchCompletion:^(NSArray<ASCollectionElement *> *elements, NSArray<ASCellNode *> *nodes) {
      ...
      [_mainSerialQueue performBlockOnMainThread:^{
        as_activity_scope(as_activity_create("collectionUpdateCommit", changeSet.rootActivity, OS_ACTIVITY_FLAG_DEFAULT));
        [_delegate dataController:self willUpdateWithChangeSet:changeSet];
